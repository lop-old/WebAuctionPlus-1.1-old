...
function session_init() {
	...
	if(function_exists($func))
		if($func() == PHP_SESSION_ACTIVE) return;
	session_start();
...
// database functions
function RunQuery($query,$_file='',$_line=0) {
	global $db,$num_queries;
	if(!$db) {ConnectDB();
...
//// send email
//function send_mail($to_email,$body,$subject,$from_email,$from_name='') {
//	$headers='';
//	// Common Headers
...
//	while(substr($cookie_domain,0,1)=='.')
//		$cookie_domain=substr($cookie_domain,1);
//	while(substr($cookie_domain,-1)=='.')
